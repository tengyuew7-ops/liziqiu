...
这里必须使用“普通云函数”，不要改成 Web/HTTP 云函数。`CLOUDBASE_APIKEY` 必须使用 `service_role` API Key，不能使用 Publishable/`anon` Key。函数在变量缺失、为空或首尾包含空白时会在冷启动阶段直接失败；错误消息只包含变量名，不包含密钥值。HTTP 网关只负责把公开路由转发给该普通云函数。
